...
build_keyboard() {
    ...
    # Build Left
    west build -p always -b "$BOARD" -d "build/$SHIELD/left" -- -DSHIELD="${SHIELD}_left" -DZMK_CONFIG="$CONFIG_PATH"

    # Build Right
    west build -p always -b "$BOARD" -d "build/$SHIELD/right" -- -DSHIELD="${SHIELD}_right" -DZMK_CONFIG="$CONFIG_PATH"
