...
** [ ] Implement Combo for Layer Toggling (`&tg`) or Turning On (`&to`)
   ...
   - **Example Combo Structure:**
     #+begin_src dts
     combo_za_gaming_toggle: {
         timeout-ms = <50>;
         key-positions = <POS_Z> <POS_A>; // Replace with actual key positions for Z and A
         bindings = <&tg GAMING>;
         layers = <BASE>; // Only active on the base layer
     };
     #+end_src
...
** [ ] Port Missing QMK Features
   ...
   - [X] **One-Shot Mods on Thumbs:** Your QMK thumb clusters use One-Shot Mods extensively. This is a major ergonomic feature to port.
     we want osm as a combo i think, not on thumbs. sofle can use them on thumbs, to many keys for it to matter anyway.
...
** [X] Implement advanced layer management
   ...
   - [X] **Long-press for ADJUST layer:**
     - [X] Create a `hold-tap` behavior for 'Z' with a 2-second hold to activate `&mo ADJUST`.
     - [X] Create a `hold-tap` behavior for '/' with a 2-second hold to activate `&mo ADJUST`.
...
** [X] Implement Advanced Key Behaviors
   ...
   - [X] **Smart Key Repeat:** Add key repeat to home-row mods. This allows a letter to repeat on a double-tap-and-hold, which is not possible with a standard hold-tap.
     this does work the timing is just really tight, i think the ai suggestions got this confused with the repeat last key function that is useful on smaller boards
...
** [ ] Configure Sofle Hardware
   ...
   - [X] Add RGB underglow support to indicate the current layer.
     rgb code is in /backups/ that was working.

** [X] Integrate external modules
   - [-] **`zmk-tri-state`:**
     this can be done in base zmk now, not sure if this module is important?
     maybe it adds other features.
     - [ ] Add the module to `west.yml`.
     - [ ] Implement a one-handed Alt-Tab "swapper" (Swapper).
     - [X] Use it to create the `ADJUST` tri-layer.
...
** [X] Explore advanced ZMK behaviors (QMK Equivalents)
    ...
    - [X] **Dynamic Macros:** Investigate and enable `CONFIG_ZMK_MACRO_DYNAMIC=y` for on-the-fly macro recording.
      these are not implemented in this version, wait for update then we can do them
...
** [X] Configure BIOS/Bootloader Compatibility (NKRO)
   ...
   - [X] ACTION: Add `CONFIG_ZMK_HID_REPORT_TYPE_NKRO=y` to the `.conf` files if NKRO is desired.
     in this case... i thinhk we concede to the almight bios and just keep a spare keyboard around (oh heavens no how would we do that...) and have nkro kuz its overall better to have.

** [X] Merge `wrappertest` branch

** [X] ZMK Studio Integration
i wont be using studio ever so dont need it
   - [X] Add the ZMK Studio snippet to `build_all.sh`.
   - [X] Configure a `&studio_unlock` key in the keymap for real-time keymap changes.
