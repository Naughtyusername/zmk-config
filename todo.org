...
** [ ] Port Missing QMK Features
   ...
   - [ ] **"J + K" Combo for Escape:** Your QMK config uses `J+K` for `Escape` (Vim style).
...
** [ ] Add new combos and tap-dances
   ...
   - [ ] **Combos (QMK's "Chordal"):**
     ...
     - [ ] **`A + S` → `Backspace`**
       ...
       combo_as_bspc {
           timeout-ms = <50>;
           key-positions = <P1 P2>; // Replace P1 P2 with actual key positions for A and S
           bindings = <&kp BSPC>;
           layers = <BASE>;
       };
       #+end_src
     - [ ] **`L + ;` → `Enter`**
       ...
       combo_lscln_ent {
           timeout-ms = <50>;
           key-positions = <P3 P4>; // Replace P3 P4 with actual key positions for L and ;
           bindings = <&kp RET>;
           layers = <BASE>;
       };
       #+end_src
     - [ ] **`F + G` → `_` (Underscore)**
       ...
       combo_fg_underscore {
           timeout-ms = <50>;
           key-positions = <P5 P6>; // Replace P5 P6 with actual key positions for F and G
